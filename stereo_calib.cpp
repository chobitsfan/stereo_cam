...
// OR ELSE HARTLEY'S METHOD
 ...
 // from the fundamental matrix
    ...
    cv::cuda::GpuMat g_rmap[2][2];
    g_rmap[0][0] = cv::cuda::GpuMat(rmap[0][0]);
    g_rmap[0][1] = cv::cuda::GpuMat(rmap[0][1]);
    g_rmap[1][0] = cv::cuda::GpuMat(rmap[1][0]);
    g_rmap[1][1] = cv::cuda::GpuMat(rmap[1][1]);
    ...
    for( i = 0; i < nimages; i++ )
    {
        for( k = 0; k < 2; k++ )
        {
            ...
            cv::cuda::GpuMat g_rimg(g_img.size(), g_img.type());
            
            //remap(img, rimg, rmap[k][0], rmap[k][1], INTER_LINEAR);
            cv::cuda::remap(g_img, g_rimg, g_rmap[k][0], g_rmap[k][1], INTER_LINEAR);
            g_rimg.download(rimg);
